...
NOMBRES_CLUSTERS = {
    0: "Principiante Severo",    1: "Lector Independiente",
    2: "Principiante Severo",    3: "Avanzado Emergente",
    4: "Principiante Severo",    5: "Intermedio Progresivo",
    6: "Elemental en Desarrollo",7: "Lector Independiente",
    8: "Intermedio Bajo",        9: "Avanzado Consolidado"
}
...
if vista == "🏫 Vista Educador":
    ...
    with col2:
        tasa_media = df_usuarios['tasa_completado'].mean()
        st.metric("Tasa completado media", f"{tasa_media*100:.1f}%")
    with col3:
        aciertos_medio = df_usuarios['aciertos_comprension'].mean()
        st.metric("Aciertos comprensión", f"{aciertos_medio*100:.1f}%")
    with col4:
        nivel_medio = df_usuarios['nivel_preferido'].mean()
        st.metric("Nivel medio usuarios", f"{nivel_medio:.1f}/5")
    ...
    with tab1:
        ...
        with col1:
            st.metric("Coeficiente de Silueta", f"{silhouette_val:.4f}", "✅ Supera objetivo ≥ 0.6")
        with col2:
            st.metric("Clusters identificados", "10", "Perfiles diferenciados")
    ...
    with tab2:
        ...
        with col2:
            ...
            fig_bar = px.bar(
                x=perfil_counts.values,
                y=perfil_counts.index,
                orientation='h',
                title="Usuarios por perfil base",
                labels={'x': 'Número de usuarios', 'y': 'Perfil'},
                color=perfil_counts.values,
    ...
    with tab3:
        ...
        fig_heat = go.Figure(data=go.Heatmap(
            ...
            texttemplate="%{text}",
            showscale=True,
            colorbar=dict(title="Sesiones")
        ))
        fig_heat.update_layout(
            title="Sesiones de lectura por perfil y semana",
            xaxis_title="Semana",
            yaxis_title="Perfil de usuario",
            height=450
        )
        st.plotly_chart(fig_heat, use_container_width=True)
        st.caption("Cada celda muestra el número de sesiones de lectura completadas por perfil en cada semana.")

    # TAB 4 — Importancia características
    with tab4:
        ...
        if modelo is not None:
            try:
                ...
                fig_imp = px.bar(
                    df_imp, x='Importancia', y='Característica',
                    orientation='h',
                ...
                st.info("Cargando importancia de características desde valores de referencia del modelo entrenado.")
                feature_names = ['Palabras por frase', 'Sílabas por palabra', 'Longitud media palabra', 'Ratio palabras largas', 'Índice INFLESZ']
                importancias = [0.31, 0.27, 0.18, 0.14, 0.10]
                df_imp = pd.DataFrame({'Característica': feature_names, 'Importancia': importancias}).sort_values('Importancia', ascending=True)
                fig_imp = px.bar(df_imp, x='Importancia', y='Característica', orientation='h', color='Importancia', color_continuous_scale='Blues')
                fig_imp.update_layout(showlegend=False, height=350)
                st.plotly_chart(fig_imp, use_container_width=True)
        else:
            st.info("Modelo no encontrado. Mostrando valores de referencia.")
            feature_names = ['Palabras por frase', 'Sílabas por palabra', 'Longitud media palabra', 'Ratio palabras largas', 'Índice INFLESZ']
            importancias = [0.31, 0.27, 0.18, 0.14, 0.10]
            df_imp = pd.DataFrame({'Característica': feature_names, 'Importancia': importancias}).sort_values('Importancia', ascending=True)
            fig_imp = px.bar(df_imp, x='Importancia', y='Característica', orientation='h', color='Importancia', color_continuous_scale='Blues')
...
# ============================================================================
    ...
    cluster_usuario = int(datos_usuario['cluster'])

    # Info del usuario
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("Nivel actual", f"{nivel_usuario}/5", NOMBRES_NIVELES.get(nivel_usuario, ""))
    with col2:
        st.metric("Textos completados", f"{datos_usuario['tasa_completado']*100:.0f}%")
    with col3:
        st.metric("Comprensión media", f"{datos_usuario['aciertos_comprension']*100:.0f}%")
    with col4:
        st.metric("Sesiones por semana", f"{datos_usuario['sesiones_semana']:.0f}")
    ...
    recomendaciones = {
        ...
        4: [
            {"titulo": "Novela narrativa adulto — selección", "nivel": "Nivel 4", "tema": "Narrativa"},
            {"titulo": "Artículos de divulgación general", "nivel": "Nivel 4", "tema": "Divulgación"},
        ],
        5: [
            {"titulo": "Artículos científicos adaptados", "nivel": "Nivel 5", "tema": "Ciencia"},
    ...
    for i, (col, texto) in enumerate(zip(cols, textos_rec)):
        with col:
...
# ============================================================================
    ...
    st.title("🔍 Clasificador de Nivel de Lectura")
            ...
            if features:
                ...
                if modelo is not None:
                    try:
                        ...
                        nivel_predicho = int(modelo.predict(X_pred)[0])
                ...
                # Fallback por INFLESZ si el modelo falla
                ...
                fig_radar.update_layout(
                    ...
                    title="Perfil lingüístico del texto",
                    height=350
                )
                ...
                descripciones = {
                    1: "✅ Texto muy accesible. Adecuado para personas con DI que están comenzando su proceso lector. Frases cortas, vocabulario cotidiano.",
                    2: "✅ Texto accesible. Adecuado para lectores en nivel básico. Vocabulario simple con alguna palabra nueva.",
                    3: "⚠️ Texto de dificultad media. Requiere cierta fluidez lectora. Puede necesitar adaptación para personas con DI.",
                    4: "⚠️ Texto complejo. Recomendado para lectores con buena comprensión. No adecuado sin adaptación para DI.",
